<?php 

    include('../include/modulos.php');
    

    $depa = $_POST['id'];


    if (!empty($_POST['nombre'])) {
        $apellido = $_POST['apellido'];
        $nombre=$_POST['nombre'];   
        $cedula=$_POST['cedula'];   
        $edad=$_POST['edad']; 
    }else{
        $apellido = 'N/A';
        $nombre='N/A';   
        $cedula=0;   
        $edad=0; 
    }
     
    //$productos = implode(",", $_POST['buscador']);
    $consul=pg_query($con,"SELECT cedula from familiares where cedula=$cedula");
    $resul=pg_fetch_array($consul);

    $existeCedula=$resul['cedula'];

    $consulR=pg_query($con,"SELECT reserva from departamentos where iddepa=$depa");
    $resulR=pg_fetch_array($consulR);

    $existeReserva=$resulR['reserva'];

    if ($edad <= 8) {
        
        $query= pg_query("INSERT into familiares (nombres, apellidos, cedula, edad, grupo, departamento, invitado)
                        values('$nombre', '$apellido', NULL, $edad, 1, $depa, 1)");

        $reserva=pg_query("INSERT INTO reserva_cliente(reserva, nombre, apellido, cedula, edad, iddepa)
                            VALUES ('$existeReserva', '$nombre', '$apellido', NULL,$edad, $depa);");

            if ($query) {
                
                header('Location: ../vista/inicioHuespedInver.php');
            }else{
                echo "<h1 style='color:#fff;'>Error al Ingresar</h1>";
            }
    }else{
        if ($existeCedula!=$cedula) {

            $query= pg_query("INSERT into familiares (nombres, apellidos, cedula, edad, grupo, departamento, invitado)
                        values('$nombre', '$apellido', $cedula, $edad, 1, $depa, 1)");
        
            pg_query("UPDATE departamentos set estatus=0 where iddepa=$depa");

            $reserva=pg_query("INSERT INTO reserva_cliente(reserva, nombre, apellido, cedula, edad, iddepa)
                            VALUES ('$existeReserva', '$nombre', '$apellido', $cedula,$edad, $depa);");

            if ($query) {
                
                header('Location: ../vista/inicioHuespedInver.php');
            }else{
                echo "<h1 style='color:#fff;'>Error al Ingresar</h1>";
            }
        }else{

            $query= pg_query("UPDATE familiares set estatus=1, grupo=1, departamento=$depa, asiste=1 where cedula=$cedula;");

            $reserva=pg_query("INSERT INTO reserva_cliente(reserva, nombre, apellido, cedula, edad, iddepa)
                            VALUES ('$existeReserva', '$nombre', '$apellido', $cedula,$edad, $depa);");
            
            //$data=1;
            //header('Location: ../vista/inicioHuespedInver.php?cd='.$data);
            pg_query("UPDATE departamentos set estatus=0 where iddepa=$depa");
            if ($query) {
                
                header('Location: ../vista/inicioHuespedInver.php');
            }else{
                echo "<h1 style='color:#fff;'>Error al Ingresar</h1>";
            }

        }
    }

 ?>