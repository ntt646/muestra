	...
	class PDF extends FPDF
{
	...
	$clase=$_GET['clase'];
		...
		$fecha=pg_query("SELECT fc.fecha_inicio as fi, fc.fecha_fin as ff , fc.observacion,
						(select rs.reserva from reserva rs where rs.fecha=fc.fecha_inicio and dp.clasificacion=1) as reserva
						from fechas fc
						inner join departamentos dp on dp.iddepa=fc.departamento
						left join reserva rs on rs.fecha=fc.fecha_inicio
						where fc.fecha_inicio between '$fi' and '$ff'
						and fc.fecha_fin between '$fi' and '$ff'
						and dp.clasificacion=$clase
						");
...
if ($clase==1) {
	...
	$pdf->cell(42,10,'Observcion', 1, 1, 'C',0);
}else{
	$pdf->cell(42,10,'Fecha', 1, 0, 'C',0);
	$pdf->cell(42,10,'Observcion', 1, 1, 'C',0);
}
...
while ($date= pg_fetch_assoc($fecha)) {
	...
	$consulVehi= pg_query("SELECT vh.placa,vh.marca, vh.modelo, vh.color,
							--fc.fecha_inicio as fi,	fc.fecha_fin as ff,
							vh.observacion,dp.codigo as depa
					    	from vehiculo vh
							inner join departamentos dp on dp.iddepa=vh.iddepa
							left join fechas fc on fc.departamento=dp.iddepa and fc.fecha_inicio=vh.fecha
